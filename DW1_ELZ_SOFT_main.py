...
import DW1_ELZ_SOFT_func as DW1ELZ
import DW2_func as DW2F


if __name__ == "__main__":
    """El Zein Portion 2 の二値・冗長化 SOFT 版を実行・評価する。"""
    ...
    print(f"[ElZein-SOFT] 面に未参照の孤立頂点を {len(isolated_indices)} 個除去しました。")
    ...
    try:
        ...
        raise RuntimeError(f"ElZein-SOFTの埋め込みに失敗しました: {error}") from error
    embed_time = time.time() - start_embed

    # OP. ノイズ攻撃
    # xyz_after = DW2F.noise_addition_attack(
    #     xyz_after, noise_percent=0.75, mode="gaussian", seed=42
    # )

    # OP. スムージング攻撃
    # xyz_after = DW2F.smoothing_attack(
    #     xyz_after, lambda_val=0.2, iterations=30, k=6
    # )

    # OP. 切り取り攻撃
    # xyz_after = DW2F.cropping_attack(
    #     xyz_after, keep_ratio=0.5, mode="axis", axis=0
    # )

    # OP. ダウンサンプリング攻撃
    # xyz_after = DW2F.downsampling_attack(
    #     xyz_after, mode="voxel", voxel_size_percent=2.0, seed=42
    # )
    ...
    DW2F.bitarray_to_image(
        extracted_bits, n=n, save_path="elzein_soft_recovered.bmp"
    )

    # 8. 固有評価
    print("[ElZein-SOFT] Portion 2-only / grouped redundant majority voting")
    print(f"[ElZein-SOFT] scaling factor a: {a}")
