...
IMAGE_PATH = "watermark32.bmp"                # 埋め込む透かし画像
WATERMARK_SIZE = 32                           # 埋め込む画像のサイズ (nxn)

# --- 攻撃に関する設定 ---
ATTACKS = [
    ("noise", [0.5, 1.0, 1.5, 2.0, 2.5, 3.0]),
...
CLUSTER_POINTS_PROPOSED = [1000, 2000, 3000]  # 検証する提案手法の1クラスタあたりの点数パターンのリスト
...
def apply_attack(xyz_after, attack_type, attack_param, seed):
    if attack_type == "noise":
        return STG50F.noise_addition_attack(xyz_after, noise_percent=attack_param, mode=NOISE_MODE, seed=seed)
    elif attack_type == "smoothing":
        try:
            return STG50F.smoothing_attack(xyz_after, lambda_val=SMOOTHING_LAMBDA, iterations=int(attack_param), k=SMOOTHING_K)
        except AttributeError:
            return xyz_after.copy()
    elif attack_type == "cropping":
        try:
            xyz_cropped = STG50F.cropping_attack(xyz_after, keep_ratio=attack_param, mode=CROPPING_MODE, axis=CROPPING_AXIS)
            # 抽出関数が正しく動作するように、元の頂点順序やインデックス数に再配置（補間等）を行う
            return STG50F.reorder_point_cloud(xyz_cropped, xyz_after)
        except AttributeError:
            return xyz_after.copy()
    else:
        return xyz_after.copy()
...
def run_embedding_baseline(pcd_before, xyz_orig, medium_indices, watermark_bits, a):
    ...
    metrics = STG50F.evaluate_imperceptibility(pcd_before, pcd_after, verbose=False)
...
def attack_and_extract_baseline(xyz_orig, xyz_after, watermark_bits, attack_type, attack_param, seed):
    ...
    try:
        extracted_bits = STG50B.extract_watermark_baseline(xyz_att, xyz_orig, n_points=len(watermark_bits), k=BASELINE_KNN_K, verbose=False)
    except Exception as e:
        print(f"Baseline抽出エラー: {e}")
        extracted_bits = []
    _, ber = STG50F.evaluate_robustness(watermark_bits, extracted_bits, verbose=False)
    return ber
...
def main():
    ...
    try:
        watermark_bits = STG50F.image_to_bitarray(IMAGE_PATH, n=WATERMARK_SIZE)
    except Exception as e:
    ...
    medium_indices = STG50B.local_feature_clustering(xyz_orig, k=BASELINE_KNN_K, verbose=False)
    init_a = 0.001
    ...
    for cp in CLUSTER_POINTS_PROPOSED:
        ...
        labels_prop = STG50F.kmeans_cluster_points(xyz_orig, cluster_point=cp, seed=42)
